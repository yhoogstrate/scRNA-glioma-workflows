...
```{r}
tmp <- celltype_markers |> 
  dplyr::filter(celltype == "TAM") |> 
  dplyr::pull(markers)


plt <- geneset_component_association(
  object = obj__van_hijfte__sample_y,
  features = tmp,
  k.components = obj__van_hijfte__sample_y@misc$pca.informative.components) |> 
  dplyr::mutate(score = -log10(plt$p.val))

ggplot(plt, aes(x=PC, y=score)) +
  geom_point()
...
```{r}
tmp <- celltype_markers |> 
  dplyr::filter(celltype == "Cell cycling") |> 
  dplyr::pull(markers)
