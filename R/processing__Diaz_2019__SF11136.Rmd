...
```{r}
obj__diaz_2019__SF11136$celltype.annotated = ifelse(obj__diaz_2019__SF11136$seurat_clusters %in% c(11),"endothelial",obj__diaz_2019__SF11136$celltype.annotated)
...
plt <- data.frame(slot = names(obj__diaz_2019__SF11136@meta.data)) |> 
  dplyr::filter(slot  == 'celltype.annotated' | grepl("^reclass\\.LDA\\.",slot))

plt <- obj__diaz_2019__SF11136@meta.data |>
  dplyr::select(plt$slot) |> 
  tidyr::pivot_longer(cols=-'celltype.annotated') |> 
  dplyr::group_by(name) |> 
  dplyr::mutate(x = order(order(value))) |> 
  dplyr::ungroup()


ggplot(plt, aes(x = x, y=value, col=celltype.annotated)) +
