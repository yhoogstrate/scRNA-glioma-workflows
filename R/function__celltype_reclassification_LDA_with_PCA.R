...
celltype_reclassification_lda_with_pca <- function(seurat_object, marker_features, n.components, slot_labels = "celltype.annotated", prefix="reclass.LDA_PCA.") {
  out <- data.frame()
  ...
  pca <- features |> 
    ...
    purrr::pluck('x') |> 
    as.data.frame() |> 
    dplyr::select(paste0('PC', 1:n.components))
