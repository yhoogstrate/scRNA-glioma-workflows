...
config <- list(
  ...
  tumor.type = 'Astrocytoma, IDH-mut'
)
...
```{r, set and confirm}
obj__yuan__PJ016$celltype.annotated.venteicher = ifelse(obj__yuan__PJ016$seurat_clusters %in% 
                                            c(3,0,2,6,1,4),
                                          "Astro. program",obj__yuan__PJ016$celltype.annotated)
```
...
```{r}
en_pe = obj__yuan__PJ016[,obj__yuan__PJ016$seurat_clusters %in% c(11)]
...
obj__yuan__PJ016$celltype.annotated = ifelse(colnames(obj__yuan__PJ016) %in% en.umis,"endothelial", obj__yuan__PJ016$celltype.annotated)
rm(en.umis)

obj__yuan__PJ016$celltype.annotated = ifelse(colnames(obj__yuan__PJ016) %in% pe.umis,"pericyte", obj__yuan__PJ016$celltype.annotated)
rm(pe.umis)
...
```{r}
# top features
obj__yuan__PJ016@reductions$pca@feature.loadings |> 
  as.data.frame() |> 
  dplyr::arrange(desc(abs(PC_1))) |> 
  dplyr::select("PC_1") |> 
  head(n=25)

FeaturePlot(obj__yuan__PJ016, reduction="umap", features=c(""))
...
Seurat::FeaturePlot(obj__yuan__PJ016, reduction="umap", features=c("ID4"))
Seurat::FeaturePlot(obj__yuan__PJ016, reduction="umap", features=c("GABRG1"))
```
...
Seurat::FeaturePlot(obj__yuan__PJ016, reduction="umap", features=c("TMEM108"))
Seurat::FeaturePlot(obj__yuan__PJ016, reduction="umap", features=c("TMEM163"))
Seurat::FeaturePlot(obj__yuan__PJ016, reduction="umap", features=c("PRICKLE1"))
```
...
Seurat::FeaturePlot(obj__yuan__PJ016, reduction="umap", features=c("C1S"))
Seurat::FeaturePlot(obj__yuan__PJ016, reduction="umap", features=c("DCN"))
Seurat::FeaturePlot(obj__yuan__PJ016, reduction="umap", features=c("BTNL9"))
Seurat::FeaturePlot(obj__yuan__PJ016, reduction="umap", features=c("CDA"))
```
...
```{r}
obj__yuan__PJ016 <- celltype_reclassification_lda(
  seurat_object = obj__yuan__PJ016,
...
```{r}
plot_sorted_classifier_scores(obj__yuan__PJ016,'celltype.annotated' ,"^reclass.LDA.")
```
...
```{r}
obj__yuan__PJ016 <- celltype_reclassification_lda_with_pca(
  seurat_object = obj__yuan__PJ016,
  celltype_markers |> dplyr::filter(celltype != "cell cycling") |> dplyr::pull(markers),
  20,
  "celltype.annotated"
...
```{r}
plot_sorted_classifier_scores(obj__yuan__PJ016,'celltype.annotated' ,"^reclass.LDA_PCA.")
```
...
Seurat::FeaturePlot(obj__yuan__PJ016,
  reduction = "umap",
  features = colnames(obj__yuan__PJ016@meta.data)[grepl("^reclass.LDA_PCA.",colnames(obj__yuan__PJ016@meta.data))],
  label = F,
...
```{r}
obj__yuan__PJ016 <- celltype_reclassification_pslda(
  seurat_object = obj__yuan__PJ016,
  celltype_markers |> dplyr::filter(celltype != "cell cycling") |> dplyr::pull(markers),
  20,
  "celltype.annotated"
...
obj__yuan__PJ016 = flag_uncertain_classifications(obj__yuan__PJ016, "PSLDA")
```

```{r}
#plot_sorted_classifier_scores(obj__yuan__PJ016,'celltype.annotated' ,"^reclass\\.PSLDA\\.")
plot_sorted_classifier_scores(obj__yuan__PJ016,'reclass.PSLDA.class' ,"^reclass\\.PSLDA\\.", "noskip$")
```
...
Seurat::FeaturePlot(obj__yuan__PJ016,
  reduction = "umap",
  features = colnames(obj__yuan__PJ016@meta.data)[grepl("^reclass\\.PSLDA\\.",colnames(obj__yuan__PJ016@meta.data))],
  label = F,
  cols = c("lightgrey","red", "blue")
)
```
...
```{r}
plot_sorted_classifier_scores(obj__yuan__PJ016,'celltype.annotated' ,"^reclass\\.PSLDA\\.") + 
  geom_hline(yintercept = obj__yuan__PJ016@misc$reclass.PSLDA.min.scores$`pericyte`, lwd=0.5, col="darkgray")
```
...
```{r}
plot_sorted_classifier_scores(obj__yuan__PJ016,'celltype.annotated' ,"^reclass\\.PSLDA\\.") + 
  geom_hline(yintercept = obj__yuan__PJ016@misc$reclass.PSLDA.min.scores$`oligodendrocyte`, lwd=0.5, col="darkgray")
```
...
```{r}
plot_sorted_classifier_scores(obj__yuan__PJ016,'celltype.annotated' ,"^reclass\\.PSLDA\\.") + 
  geom_hline(yintercept = obj__yuan__PJ016@misc$reclass.PSLDA.min.scores$`tumor`, lwd=0.5, col="darkgray")
```
...
```{r}
FeaturePlot(obj__yuan__PJ016, features=c("TMEM144"))
